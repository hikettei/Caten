...
(defmethod dequantize ((type-id (eql :q8_0)) aligned-buffer tensor-info)
  ...
  (let* ((size (apply #'* (tensor-info-dimensions tensor-info)))
         (out (make-array size :element-type 'single-float))
         (num-blocks (ceiling size 32))
         (idx 0))
    (dotimes (bl num-blocks)
      (let* ((scale (readf16-le aligned-buffer)))
        (let ((elements-in-block (min 32 (- size idx))))
          (dotimes (i elements-in-block)
            (let ((qval (read8-le aligned-buffer)))
              (setf (aref out idx) (* qval scale))
              (incf idx))))))
    out))
