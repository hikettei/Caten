		  ...
		  (let ((x (rand ',shape1))
			...
			(proceed (!matmul x y)))
                    (when (caten/codegen/backend:jit-mode-p)
                      (ok (= 1 (n-kernels (caten (!matmul x y)))) "In-place")))))))
		...
		(with-given-dtype ((:float32 . "float32"))
		  (let ((x (rand ',shape1))
			...
			(proceed (!matmul x (!transpose y -1 -2))))
                    (when (caten/codegen/backend:jit-mode-p)
                      (ok (= 1 (n-kernels (caten (!matmul x (!transpose y -1 -2))))) "In-place")))))))
  (def test-squared-2d-matmul-t1 (64 64) (64 64))
  (def test-2d-matmul-t1 (32 64) (128 64))
  (def test-broadcast-matmul-left-t1 (2 10 30) (20 30))
...
(macrolet ((def (name shape1 shape2)
		...
		(with-given-dtype ((:float32 . "float32"))
		  (let ((x (rand ',shape1))
			...
			(proceed (!matmul (!transpose x -1 -2) y)))
                    (when (caten/codegen/backend:jit-mode-p)
                      (ok (= 1 (n-kernels (caten (!matmul (!transpose x -1 -2) y)))) "In-place")))))))
  ...
  (def test-broadcast-matmul-right-t2 (10 20) (2 10 20))
