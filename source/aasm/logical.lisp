...
(defun %where (out condition x y &key (id (gensym "WID")))
  "id = {out + where(condition, x{true-then}, y{false-then})}"
  (declare (type node condition x y))
  (emit (make-node :TernaryOps :WHERE (list id) (list (node->id out) (node->id condition) (node->id x) (node->id y)))))
