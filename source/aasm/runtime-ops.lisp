...
(defun $kernel (read-kernels read-args kernel-info &key (dtypes) (out (gensym "K")))
  "
```
...
"
  ...
  (emit (make-node :RUNTIME :KERNEL (list out) (map 'list #'node->id1 (append read-kernels read-args)) :n-kernel-args (length read-kernels) :kernel-info kernel-info :dtypes dtypes)))
