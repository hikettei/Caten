		    ...
		    (multiple-value-bind (replace-rule matched)
			  ...
			  (dolist (r matched)
			    ...
			    (when (and (not (eql r (node-id ,node-top))) (id->node ,graph r))
				  ...
				  (remnode ,graph r))))))
		        t)))
                  (,apply-bind1 (graph &aux (changed-p nil) (index 0))
                    (dolist (node (graph-nodes graph))
                      (declare (type fixnum index))
                      (when (,simplifier-bind node index)
                        (setf changed-p t))
                      (incf index))
                    changed-p)
                  (,apply-bind2 (id)
                      ...
                      (when node
                        (when (null (find (the symbol id) (the list ,seen) :test #'eq))
                          ...
                          (when (,simplifier-bind node -1)
                            (some #',apply-bind2 (node-reads node))
                            (return-from ,apply-bind2 t))
                          (some #',apply-bind2 (node-reads node))))))))
           (if ,fast-graph-p
               (loop while (and (some #',apply-bind2 (graph-outputs ,graph))
