...
(defun r/schedule (name args)
  ;; :idx = (T12 -> 12)
  (make-node :Render :SCHEDULE nil nil :name name :args args :idx (parse-integer (subseq name 1))))
  ...
  (let ((new-graph nil))
    (labels ((lower (object)
		 ...
		 ((User :name name :args args)
		  (push (r/schedule name args) new-graph))
    ...
    (let ((new-graph (reverse new-graph)))
      (flet ((ts (n) (find (format nil "T~a" n) new-graph :test #'equal :key #'(lambda (x) (and (eql (node-type x) :SCHEDULE) (getattr x :name))))))
