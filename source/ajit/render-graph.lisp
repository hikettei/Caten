...
(defun r/funcall-string (name &rest args)
  (assert (every #'expr-p args))
  (make-node :Render :FUNCALL nil nil :name name :args args :idx name))
(defun r/funcall-symbol (name &rest args)
  (assert (every #'expr-p args))
  (make-node :Render :FUNCALL nil nil :fname name :args args))
