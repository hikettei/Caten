...
(macrolet ((impl (kw op)
	     `(defmethod %impl ((device-id (eql :lisp)) (op (eql ,kw)) graph node args &aux (max (caten/common.dtype:dtype/max (buffer-dtype (car args)))) (min (caten/common.dtype:dtype/min (buffer-dtype (car args)))))
		(declare (ignorable max min))
