...
(defun %connect-expr (node args out)
  (declare (type node node))
  (assert (every #'expr-p args))
  (let ((graph (apply #'make-graph (apply #'append (list node) (map 'list (alexandria:compose #'graph-nodes #'expr-graph) args)))))
    (setf (graph-outputs graph) (list out))
    (simplify-expr (make-expr :graph graph :out node))))
...
(defun expr-const (value dtype &aux (out (gensym "w")))
  ...
  (let ((nodes (graph-nodes (with-context (_ (%fconst value :dtype dtype))))))
    ...
    (%connect-expr
     (second nodes)
     (list (%connect-expr (car nodes) nil (car (node-writes (car nodes)))))
     out)))

(macrolet ((def (name op)
             `(defun ,name (a b &aux (out (gensym "w")))
                (declare (type Expr a b))
                (%connect-expr (,op (expr-out a) (expr-out b) :id out) (list a b) out))))
