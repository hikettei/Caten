  ...
  (labels ((merge-broadcast (views list1 &key (default 1) (size) &aux (list (copy-list list1)))
             (loop for v in views
                   ...
                   if (fourth v)
                     ...
                     collect (or (pop list) (error "merge-broadcast: cannot merge shape ~a and map ~a." views list1))))
           (fixup (base-type tgt-type)
             ...
             (setf (buffer-shape tgt-type) (merge-broadcast (buffer-views base-type) (buffer-shape tgt-type)
                   ...
                   (buffer-stride tgt-type) (merge-broadcast (buffer-views base-type) (buffer-stride tgt-type) :default 0)
