...
One Schedule-Item corresponds to one kernel in GPU, `graph-schedule` must ensure that each item is merged only within the bounds that can be lowered into a single kernel.")
  (:use :cl :caten/aasm :caten/air :caten/runtime :caten/codegen/iteration :caten/aasm/expr :caten/codegen/helpers :caten/codegen/rewriting-rules)
...
(defnode (:GRAPH :Schedule-Item) ()
         ...
         :slots
         ((blueprint :type (or null Graph) :initform nil) ;; AST
          (items :type list) (items-to-cache :type list) ;; a list of tensor ops involved in the kernel / items used to generate a copy of the kernel
          (type :type (member :kernel :allocate :vmop :backward)) ;; Type of schedule-item
  ...
  (flet ((r (x)
    ...
    (format nil "{ ~a } : [ ~a <- ~a where lowered-p=~a ~a]"
            ...
            (if (eql (getattr node :type) :allocate)
                ...
                (r (node-reads node)))
            (if (getattr node :blueprint) "t" "nil")
            (if (eql :allocate (getattr node :type))
                ""
...
;; ~~ Schedule Cache ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
(defun tensor-relay-equal (a b)
  (declare (optimize (speed 3)))
  (and
   (typep a 'TensorRelay) (typep b 'TensorRelay)
   (equal (tensor-relay-shape a) (tensor-relay-shape b))
   (equal (tensor-relay-stride a) (tensor-relay-stride b))
   (equal (tensor-relay-views a) (tensor-relay-views b))
   (equal (tensor-relay-dtype a) (tensor-relay-dtype b))
   (equal (tensor-relay-inferred-permute a) (tensor-relay-inferred-permute b))))
    ...
    #'(lambda (x y)
        (and
         ...
         (let ((xt (read-type-relay x))
           ...
           (and
            (every #'(lambda (a b) (tensor-relay-equal a b)) (relay-reads xt) (relay-reads yt))
            (every #'(lambda (a b) (tensor-relay-equal a b)) (relay-writes xt) (relay-writes yt))))))
    items1 items2)))
;; ~~ Some Helpers ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
(defmethod node-reduce-axes ((node Node))
  (when (getattr node :reduction :allow-undefined t)
    (let ((write-buffer (car (relay-writes (read-type-relay node)))))
      (let ((out
              (loop for v in (tensor-relay-views write-buffer)
                    for s in (tensor-relay-shape write-buffer)
                    if (fourth v)
                      collect s
                    else
                      collect nil)))
...
(defmethod group-rank ((group Group) &aux (buff (group-get-type group)))
  (and buff (tensor-relay-nrank buff)))
...
(defun group-assert-rank (group r1 r2 view &aux (rank (max r1 r2)))
  (loop for item in (group-items group)
        do (loop for typ in (append (relay-reads (read-type-relay item)) (relay-writes (read-type-relay item)))
                 for nth upfrom 0
                 unless (or (null typ) (= 0 (tensor-relay-nrank typ)))
                   do (assert (= rank (tensor-relay-nrank typ)) ()
...
(defun apply-view-fusor (tgt-rank mask group &key (permute))
  ...
  (group-items-st-rewriter
   group
   #'(lambda (typ)
       (when (= (tensor-relay-nrank typ) tgt-rank)
         (let ((typ (copy-tensor-relay typ)))
           (let ((shp (copy-list (tensor-relay-shape typ)))
                 (str (copy-list (tensor-relay-stride typ)))
                 (views (copy-list (tensor-relay-views typ))))
             (setf (tensor-relay-shape typ)
                   (loop for b in mask
                         if b collect 1 else collect (or (pop shp) 1))
                   (tensor-relay-stride typ)
                   (loop for b in mask
                         if b collect 0 else collect (or (pop str) 1))
                   (tensor-relay-views typ)
                   (loop for b in mask
                         if b collect `(0 1 1 t) else collect (pop views))
                   (tensor-relay-nrank typ) (length (tensor-relay-shape typ))
                   (tensor-relay-inferred-permute typ) (if (tensor-relay-inferred-permute typ)
                                                     (merge-permute-from-mask (tensor-relay-inferred-permute typ) mask)
                                                     (merge-permute-from-mask (range 0 (tensor-relay-nrank typ)) mask)))
             (when (and permute (tensor-relay-inferred-permute typ) (= (length permute) (tensor-relay-nrank typ))
                        (not (equal permute (tensor-relay-inferred-permute typ))))
               (setf (tensor-relay-shape typ) (permute-list permute (tensor-relay-shape typ))
                     (tensor-relay-stride typ) (permute-list permute (tensor-relay-stride typ))
                     (tensor-relay-views typ) (permute-list permute (tensor-relay-views typ))))
             ;; Consumed all masks?
             (assert (= (tensor-relay-nrank typ) (length mask)))
             typ))))
   mask))
...
(defmethod identify-view-type ((view Node))
  ...
  (flet ((shrink-p (size view)
    ...
    (let* ((base-buffer (car (relay-reads (read-type-relay view))))
           (views (tensor-relay-views (car (relay-writes (read-type-relay view))))))
      (when (and
             (= (tensor-relay-nrank base-buffer) (getattr view :nrank))
             (some #'shrink-p (tensor-relay-shape base-buffer) views))
...
(defmethod group->schedule-item ((group Group))
  (let ((reads (group-predecessor group))
    ...
    (make-node :GRAPH :Schedule-Item writes reads :name (make-unique-schedule-name group)
               :type
               (cond
                 ((and (every #'jitable-p (group-items group)) (null full-scalar-p)) :kernel)
                 (allocate-p :allocate)
                 (t :vmop))
               :storage-id-dst writes :storage-id-src reads
...
(defmethod ctx-node-force-realize-p ((ctx Schedule-Context) (a node) (b node))
  "Ensure A and B are strongly connected components. In order to merge a and b in the same group, c must be grouped to a or b.
[a] ──────> d ──────>[b]
  \       /         /
   \     /         /
    \   /         /  ... (recursively)
  ...
  (symbol-macrolet ((->ng (return-from groups-rewrite-views-in-the-same-space nil))
    ...
    (let* ((r1 (group-rank tgt-group)) (r2 (group-rank parent-group))
      ...
      (if (broadcastable-p read-type self-type) ;; simpliy broadcastable
          (let* ((base-1 (loop for s in (tensor-relay-shape (if c self-type read-type)) if (eql s 1) collect s))
                 (mask (map 'list
                            #'(lambda (x)
                                (if (eql x 1)
                                    (if base-1
                                        ...
                                        t)
                                    nil))
                            (tensor-relay-shape (if c read-type self-type)))))
          ...
          (if (and view (some #'identity (getattr view :broadcast)))
              (let ((mask (getattr view :broadcast)))
                ...
                (when (not (= (length mask) (max r1 r2)))
                  (setf mask (map 'list #'fourth (tensor-relay-views (if c read-type self-type)))))
  ...
  (labels ((reduce-p (node)
           ...
           (broadcast-conflicts-p (node)
             (loop for read in (relay-reads (read-type-relay node))
                   for views = (tensor-relay-views read)
  ...
  (symbol-macrolet ((->ng (return-from group-mergeable-p nil))
    ...
    (when (group-reduce-dims tgt-group)
      (let ((typ (group-get-type parent-group)))
        (when (and
               (= (tensor-relay-nrank typ) (length (group-reduce-dims tgt-group)))
               (= (length (group-items parent-group)) 1)
               (eql :LOAD (node-type (car (group-items parent-group)))))
          (loop for shape in (tensor-relay-shape typ)
                for nth upfrom 0
                for dim in (group-reduce-dims tgt-group)
                for view = (nth nth (tensor-relay-views typ))
    ...
    (let ((reduce-p (identity (or (group-reduce-dims parent-group) (group-reduce-dims tgt-group))))
      ...
      (when reduce-p ;; If tgt or parent has a reduction:
        ...
        (if (= r1 r2)
            (when (tensor-relay-mergeable-p (ctx-graph ctx) (group-get-type tgt-group) (group-get-type parent-group))
      ...
      (when (= r1 r2)
        (when (tensor-relay-mergeable-p (ctx-graph ctx) (group-get-type parent-group) (group-get-type tgt-group))
  ...
  (when (every #'jitable-p (group-items group)) ;; this is only the case for jitable kernel
    (loop for predecessor in (group-predecessor group)
          ...
          if (and item alloc (eql (node-type item) :LOAD) (eql (node-type alloc) :Allocate)
                  (= 0 (tensor-relay-nrank (car (relay-writes (read-type-relay item)))))
...
(defun apply-move-after-reduction (schedule-graph)
  ...
  (labels ((%newtype (buffer)
             (make-tensor-relay
              (loop for s in (tensor-relay-shape buffer)
                    for nth fixnum upfrom 0
                    for v = (nth nth (tensor-relay-views buffer))
                    if (and (listp v) (fourth v)) ;; broadcasted
                      collect 1
                    else
                      collect s)
              (tensor-relay-stride buffer)
              (tensor-relay-dtype buffer)
              (loop for s in (tensor-relay-shape buffer)
                    for nth fixnum upfrom 0
                    for v = (nth nth (tensor-relay-views buffer))
           ...
           (%jstore (w a b base-type)
             (let ((out (make-node :Buffer :MOVE (list w) (list a b)))
                   (rel (make-relay :reads (list (%newtype base-type) (%newtype base-type)) :writes (list (%newtype base-type)))))
               (setf (node-type-relay out) rel)
               out))
    ...
    (let ((backward-nodes
      ...
      (loop for item in (graph-nodes schedule-graph)
            if (some #'(lambda (x) (find (node-id x) backward-nodes)) (getattr item :items))
              do (setf (getattr item :type) :backward)))
    schedule-graph))
  ...
  (let ((caten/aasm:*ctx* (make-graph)))
    (labels ((e (node)
             ...
             (explore (id &aux (node (id->value schedule-graph id)))
               (when (or (null node) (gethash (node-id node) visited)) (return-from explore (gethash (node-id node) visited)))
               (assert (eql (node-type node) :Schedule-Item))
               (setf (gethash (node-id node) visited) (tgensym))
               (let ((parents (map 'list #'explore (node-reads node))))
                 (ecase (getattr node :type)
                   ((:allocate :vmop :backward)
                    (mapc #'e (getattr node :items))
                    (emit ($sync parents (node-writes node) :out (gethash (node-id node) visited)))
                    (gethash (node-id node) visited))
                   (:kernel
                    (let* ((writes (map 'list #'(lambda (x) (recursively-explore-allocate base-graph x)) (node-writes node)))
                           (writes (loop for a in writes for id in (node-writes node)
                                         do (setf (node-writes a) (list id) (getattr a :_read_views) nil (getattr a :_type_relay) nil)
                                         collect a))
      ...
      (setf (graph-outputs caten/aasm:*ctx*)
            (map 'list #'explore (graph-outputs schedule-graph))))
    (verify-graph caten/aasm:*ctx*)
    (setf (graph-nodes caten/aasm:*ctx*) (tpsort-graph caten/aasm:*ctx*))
    ;; [TODO] BackwardGraphSupport
    caten/aasm:*ctx*))
