  ...
  (let ((caten/aasm:*ctx* (make-graph)))
    (labels ((newid (id)
             ...
             (explore (id &aux (node (id->value schedule-graph id)))
               (when (null node) (return-from explore))
               (when (gethash (node-id node) visited) (return-from explore (gethash (node-id node) visited)))
               (assert (eql (node-type node) :Schedule-Item))
               (setf (gethash (node-id node) visited) (tgensym))
               (let ((parents (loop for i in (map 'list #'explore (node-reads node)) if i collect i)))
                 (ecase (getattr node :type)
