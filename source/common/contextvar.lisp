...
(macrolet ((defcontext (&rest slots)
  ...
  (defcontext
    ...
    (:DEFAULT_FLOAT
     "FLOAT32" :string
     (lambda (x &aux (x (intern x "KEYWORD")))
	 (assert (find x `(:float64 :float32 :float16 :bfloat16))
		 ()
		 "Default float is one of :FLOAT64, :FLOAT32, :FLOAT16, :BFLOAT16, getting ~a" x)
	 (ecase x
	   (:float64 (setf *read-default-float-format* 'double-float))
	   (:float32 (setf *read-default-float-format* 'single-float))
	   (:float16 (setf *read-default-float-format* 'short-float))
	   (:bfloat16 (setf *read-default-float-format* 'short-float)))
	 x)
