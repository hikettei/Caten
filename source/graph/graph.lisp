;; Graph is an abstract class that represents a DAG (Directed Acyclic Graph).
;; In typical usage, you push nodes into the list stored in `nodes`, and once
;; the graph construction is complete you call `->fast-graph` to obtain a
;; faster representation. Operations such as pattern matching are then applied
;; to that fast graph.
;; Depending on the semantics of your graph, you should define a class that
;; inherits from FastGraph (e.g., TensorGraph, RuntimeGraph, ...).
(in-package :caten/graph)
...
(defclass Graph ()
  ...
  (:documentation "
A Graph instance represents a finite directed acyclic graph
    G = (V, E, O)
where
  - V is the finite set of vertices (nodes),
  - E ⊆ V × V is a set of directed edges with no directed cycles
    (∃ a topological order τ : V → {1,…,|V|} such that (u,v)∈E ⇒ τ(u) < τ(v)),
  - O ⊆ V is the distinguished set of output vertices.

Slots:
  nodes   — a list that enumerates the elements of V in insertion order.
            We expect each v∈V to appear exactly once (intended invariant).
  seen    — an auxiliary subset S ⊆ V used during traversals/analyses
            to track visitation (e.g., DFS/BFS marks).
  outputs — a list that denotes O, the output vertex set."))
...
(defmethod remnode ((graph Graph) id)
  ...
  (setf (graph-nodes graph)
	(loop for node in (graph-nodes graph)
	      unless (find id (node-writes node)) collect node)))
...
(defmethod purge-isolated-graph ((Graph graph))
  ...
  (when (graph-nodes graph)
	...
	(mapc #'helper output))
      (setf (graph-nodes graph)
	    (loop for node in (graph-nodes graph) 
		  if (find (node-id node) valid-write-ids) ;; node exists in a valid path
		    collect node)))))
