...
(defun infer-tensor-info (graph id) ;; TODO: Remove this utilities, use read_type_relay directly!
  ...
  (let ((val (id->value graph id)))
    (assert val)
    (let ((rel (node-type-relay val)))
      (when rel
        (values (tensor-relay-nrank rel) (tensor-relay-shape rel) (tensor-relay-stride rel) (tensor-relay-dtype rel) (tensor-relay-views rel))))))
...
i.e.: _gid_n = _gid_n'*coeffs[n]+offsets[n]"
  (declare (type (or symbol node) base)
...
(defun %partial-schedule (base size stride dilation offset &key (dims 0) (id (lgensym "PV_")))
