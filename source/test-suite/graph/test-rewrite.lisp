(defpackage :caten/test-suite/graph/test-rewrite
  (:use :cl :caten/graph :trivia :rove))
(in-package :caten/test-suite/graph/test-rewrite)

(defun <node> (type writes reads &rest attrs) (apply #'make-node :Testing type writes reads attrs))

(defun compare (graph expected
		&key
		  (slots `(caten/graph::writes caten/graph::reads caten/graph::type caten/graph::class))
		  (shuffle-order t))
  (flet ((eq-node (n1 n2)
	   (and
