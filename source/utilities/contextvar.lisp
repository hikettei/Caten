(defpackage :caten/utilities/contextvar
  (:nicknames :ctx)
  (:use :cl :cl-ppcre)
  (:export
   #:*ctx*
   ...
   #:getenv
   #:with-contextvar))
(in-package :caten/utilities/contextvar)
...
(macrolet ((defcontext (&rest slots)
	     (assert (every
			  ...
			  (and
			   (= (length x) 5)
			   (keywordp (car x))
			   (find (third x) `(:int :string :float))
			   (ecase (third x)
                             (:float (floatp (second x)))
		...
		(defstruct ContextVar
		  ,@(loop for slot in slots
			  ...
			  for dtype = (ecase (third slot) (:int 'fixnum) (:string 'string) (:float 'single-float))
			  collect `(,slot-name (if (uiop:getenv ,(symbol-name (car slot)))
						   (ecase ,(third slot)
						     ((:int :float)
						      (let ((val (read-from-string (uiop:getenv ,(symbol-name (car slot))))))
							(if (if (eql ,(third slot) :int) (integerp val) (floatp val))
							    val
							    (progn
							      (warn "Caten/common.contextvar: ~a should be an integer/float but got ~a, setting the default value." ',(car slot) val)
							      ,default))))
				 ...
				 (ecase ,(third slot)
				   ((:float :int)
				    (let ((val (read-from-string (format nil "~a" value))))
				      (if (if (eql ,(third slot) :int) (integerp val) (floatp val))
					  val
					  (error "Caten/common.contextvar: ~a should be an integer/float but got ~a" ',(car slot) value))))
				   (:string
  ...
  (defcontext
    ...
    (:DEBUG
     1 :int
     (lambda (x)
       (when (not (typep x '(integer 0 5))) (warn "DEBUG should be an integer from 0 to 5, got ~a, setting 0." x) (setf x 0))
       x)
     "Choose a value from 0 to 5. Gradually specifies the level of debugging when executing with JIT (If unsure, setting JIT_DEBUG >= 2 is recommended).")
    (:DEFAULT_FLOAT
     "FLOAT32" :string
