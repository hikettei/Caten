...
(defun parse-type-designator (type-form var)
  ...
  (match type-form
    ...
    (_
     (assert type-form () "defaction: Cannot infer the type of ~A.~%Provide (declare (type type_name variable ...)) form to declare the type." var))))
...
(defaction String-Test (table)
  (declare (type (:tensor :row (n) :string) table)
           (optimize (debug 1)))
  (dotimes (i n)
    (setf (aref table i) "ABCDEFG")))
